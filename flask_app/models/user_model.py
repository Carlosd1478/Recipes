...
class User:
    ...
    def validate_registration( data ):
        ...
        if len(data['first_name']) < 2:
            flash('First name must be at leaast 2 characters long.','first_name_error' )
            is_valid = False 
        if len( data['last_name']) < 2:
            flash('Last name must be at leaast 2 characters long.','last_name_error' )
            is_valid = False 
    ...
    @staticmethod
    def encypt_password( pwd, bcrypt ):
    ...
    def validate_password( pwd, encrypted_pwd, bcrypt):
        if not bcrypt.check_password_hash(encrypted_pwd, pwd):
            flash("wrong credientals", "password_login_error")
            return False
        else:
            return True